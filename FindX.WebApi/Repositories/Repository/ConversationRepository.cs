...
namespace FindX.WebApi.Repositories.Repository
{
    public class ConversationRepository : IConversationRepository
    {
        ...
        public async Task SetMessageAsSeen(Guid userid, Guid convid)
        {

            var convFillter = _convFilterBuilder.Eq(conv => conv.Id, convid);
            var conv = await _context.Conversations.Find(convFillter).SingleOrDefaultAsync();
            conv.Messages.Last().Seen = true;

            await _context.Conversations.ReplaceOneAsync(convFillter, conv);
        }
